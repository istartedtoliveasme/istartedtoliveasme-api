...
func (user *User) GetById(props GetByIdProps) types.CustomError {
	...
	userJson := types.Json[User]{
		Payload: *user,
	}
	if err = userJson.Parse(record); err != nil {
		return UserError{
...
func (user *User) GetByEmail(props GetByEmailProps) types.CustomError {
...
func (user *User) Create(props CreateProps) types.CustomError {
	tx := props.GetSession()
	cypher := "MATCH (u:User) WHERE user.email != $email" +
		" CREATE (u:User { id: $id, firstName: $firstName, lastName: $lastName, email: $email, password: $password }) " +
		"RETURN u LIMIT 1"
...
func (user *User) BindRecord(result neo4j.Result) types.CustomError {
