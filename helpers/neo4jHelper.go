...
func GetSingleRecord(result neo4j.Result) (GetSingleRecordResponse, error) {
	var jsonResponse types.Json[GetSingleRecordResponse]
	...
	if err = jsonResponse.Parse(singleProps); err != nil {
		return nil, err
	}
...
func GetAllRecords(result neo4j.Result) ([]GetSingleRecordResponse, types.CustomError) {
	var allRecords types.Json[[]GetSingleRecordResponse]
	...
	for _, record := range collections {
		var singleRecord types.Json[GetSingleRecordResponse]
		data, err := GetSinglePropsByRecord(*record)
		if parseErr := singleRecord.Parse(data); err != nil || parseErr != nil {
...
func GetSinglePropsByRecord(record neo4j.Record) (interface{}, types.CustomError) {
